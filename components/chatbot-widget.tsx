...
const linaAvatar = '/images/lina-avatar.jpeg';
...
function LinaAvatar({ size = 'md' }: { size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <span className={`relative inline-flex shrink-0 overflow-hidden rounded-[42%_58%_48%_52%/55%_45%_55%_45%] border-2 border-white bg-butter/50 shadow-sm ${sizeClass}`}>
      <Image src={linaAvatar} alt="Cece Lina Chang" fill sizes="56px" className="object-cover object-top" />
    </span>
  );
}
      ...
      {isOpen && (
        <section className="animate-chat-window-in mb-4 flex h-[min(680px,calc(100vh-7rem))] w-[calc(100vw-2rem)] max-w-sm flex-col overflow-hidden rounded-[2rem] border border-butter/45 bg-[#fffaf3] shadow-[0_24px_70px_rgba(95,54,32,0.3)]" aria-label="Chat layanan pelanggan">
          ...
          <div className="flex-1 space-y-4 overflow-y-auto bg-[radial-gradient(circle_at_12px_12px,rgba(232,184,109,0.17)_1px,transparent_1.5px)] bg-[length:22px_22px] px-4 py-5" role="log" aria-live="polite" aria-relevant="additions">
            ...
            {isSending && (
              <div className="animate-chat-bubble-in flex items-center gap-2 pl-1 text-sm text-charcoal-brown/65">
                <LinaAvatar size="sm" />
                <span className="flex items-center gap-1.5 rounded-full bg-white px-4 py-3 shadow-sm" aria-label="Lina sedang mengetik">
                  <span className="chat-typing-dot h-1.5 w-1.5 rounded-full bg-terracotta" style={{ animationDelay: '0ms' }} />
                  <span className="chat-typing-dot h-1.5 w-1.5 rounded-full bg-terracotta" style={{ animationDelay: '150ms' }} />
                  <span className="chat-typing-dot h-1.5 w-1.5 rounded-full bg-terracotta" style={{ animationDelay: '300ms' }} />
                </span>
              </div>
            )}
